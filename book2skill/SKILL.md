...
### 全自动模式 (auto)
- 用户只需提供书籍文件路径
- 自动从文件名/EPUB元数据/内容提取书名、作者、出版年
- 跳过阶段 0 的人工确认，直接进入阶段 1
- 阶段 4 测试不通过时自动回炉（最多 3 次），不中断
- 全部完成后一次性输出结果
...
4. **交互模式**: 把产出展示给用户确认:"骨架我理解对了吗?有没有你希望重点突出的方向?" 得到确认再进入阶段 1。
5. **全自动模式**: 跳过确认,直接进入阶段 1。异常时自动重试一次。
...
**实践导向要求**: 每个候选单元必须包含 `solves_problem` 字段，说明这个方法论能解决什么真实生活问题。
...
通过的进入阶段 1.75。不通过的写入 `books/<slug>/rejected/` 并附原因 — 保留审计轨迹,也允许用户事后捞回。

### 阶段 1.75 — 真实场景映射

按 `methodology/03.5-stage1.75-scenario-mapping.md`:
1. 提取核心机制：这个方法论的本质是什么？
2. 列举现代场景：在当前时代的生活中，什么问题能用这个机制解决？
3. 写用户原话：遇到这些问题的用户会怎么说？

产出：每个通过验证的单元增加 `modern_scenarios` 字段，包含 3-5 个真实生活场景和对应的用户原话。
...
- **A2 (Future Trigger)** ★: 用户在什么情境下会需要这个 → skill 的 `description` 字段
  - **实践导向**: 必须写 3-5 个真实生活场景 + 用户原话信号
  - ✅ "用户在纠结要不要辞职创业，列了一堆利弊还是下不了决心"
  - ❌ "用户需要做决策时"
- **E (Execution)**: 1-2-3 可执行步骤
  - **实践导向**: 每一步必须是"可判断是否完成"的具体动作
  - ✅ "列出 3 个你最不希望发生的结果"
  - ❌ "保持客观"
...
3. **实践导向**: 测试用例必须用真实用户会说的话，不是教科书例题
   - ✅ "我纠结要不要接这个新项目,列了一堆好处但还是没底"
   - ❌ "我需要一个决策框架"
4. 本地跑一遍,**未过的回炉重做阶段 2** — 不做"表面修补"
5. **全自动模式**: 测试不通过时自动回炉（最多 3 次），不中断流程
6. **交互模式**: 测试不通过时询问用户是否回炉
7. 全部通过后通知用户: "已完成,可一键喂给 darwin-skill 自动进化"
...
6. **实践导向**: A2 必须包含真实生活场景和用户原话信号
7. **实践导向**: E 必须是可判断是否完成的具体动作
8. **实践导向**: 测试用例必须像真实用户会说的话
...
- **全自动模式** — 用户说"全自动拆书"时,跳过所有人工确认,自动回炉
